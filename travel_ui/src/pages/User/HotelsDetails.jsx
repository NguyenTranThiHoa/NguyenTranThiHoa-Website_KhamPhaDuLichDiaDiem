...
const HotelsDetails = () => {
    ...
    const { id } = useParams();
    const [hotelsDetail, setDetailHotels] = useState({});
    ...
    const getDetailPage = useCallback(async () => {
        try {
            const responseDetail = await hotelsService.GetHotelsById(id);
            const responseRelate = await hotelsService.GetHotels(id);
            ...
            setDetailHotels(responseDetail.data);
            setHotels(responseRelate.data);
        } catch (error) {
    ...
    return (
        <>
            <Breadcrumb
                ...
                items={[
                    { title: 'Home' },
                    { title: 'Khách sạn', href: '/hotels' },
                    { title: hotelsDetail.nameHotel || 'Chi tiết' },
                ]}
            ...
            <div style={{ padding: '10px 150px' }}>
                <Row gutter={16} justify="center">
                    <Col xs={20} sm={12} style={{ textAlign: 'center' }}>
                        <Image
                            src={hotelsDetail.imageHotel ? `https://localhost:7054/api/images/${hotelsDetail.imageHotel}` : ""}
                            alt={hotelsDetail.nameHotel}
                ...
                <div style={{ padding: '0px 20px', textAlign: 'center' }}>
                    <p>
                        Tên khách sạn: <span style={{ fontSize: '25px' }}>{hotelsDetail.nameHotel} </span>
                    </p>
                    <p>
                        Tên địa điểm: {getLocationName(hotelsDetail.locationID)}
                    </p>
                    <p>
                        Địa chỉ: Tỉnh {hotelsDetail.provinceHotel}, {hotelsDetail.cityHotel}
                    </p>
                    <p>
                        Giá thuê phòng: {hotelsDetail.pricePerNight} VNĐ/1 người
                    </p>
                    <p>
                        Điểm đánh giá: {hotelsDetail.ratingHotel} 
                        <Rate allowHalf value={hotelsDetail.ratingHotel} disabled style={{ color: '#fadb14' }} />
                    </p>
                </div>
